...
class IsoTexBlock < Block
  ...
  def initialize(col, row, x_tiles, y_tiles, height, img, img_gap)
    ...
    @imgs = (0...(x_tiles + y_tiles - 1)).map do |i|
      image.subimage((i >= x_tiles ? (i + 1) : i) * IMG_SLICE_OFFSET, 0,
                     (i == x_tiles - 1 ? 2 : 1) * IMG_SLICE_OFFSET, image.height)
    end
    @img_gap = img_gap
  ...
  def draw(map)
    ...
    x = pos.x - ((@y_tiles - 1) * Graphics::TILE_WIDTH / 2) + @img_gap.x
    y = pos.y - @height * Physics::V_UNIT + @img_gap.y
    @imgs.each_with_index do |img, i|
      img.draw(x + (i >= @x_tiles ? (i + 1) : i) * Graphics::TILE_WIDTH / 2, y,
